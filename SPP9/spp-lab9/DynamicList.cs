...
namespace spp_lab9
{
    public class DynamicList<T> : IEnumerable<T>
    {
        ...
        public IEnumerator<T> GetEnumerator()
        {
            ...
            {
                yield return Items[i]; //Наличие оператора yield return превращает метод в итератор
            }
        }

        IEnumerator IEnumerable.GetEnumerator() //Предоставляет итератор, который поддерживает простой перебор коллекции указанного типа.
        {
